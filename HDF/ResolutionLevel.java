...
public class ResolutionLevel {
   ...
   public ResolutionLevel(int index, int baseWidth, int baseHeight, int baseDepth,
      ...
      //image size won't nessessarly be an exact factor of base size
      reductionFactorX_ = (int) Math.round( baseWidth / (double) imageSizeX_);
      reductionFactorY_ = (int) Math.round( baseHeight /  (double) imageSizeY_);
      reductionFactorZ_ = (int) Math.round( baseDepth /  (double) imageSizeZ_);
      calculateBlockSize();
   }
