...
public class ResolutionLevelMaker {
   ...
   private static void addResLevels(LinkedList<ResolutionLevel> resLevels, int baseSizeX, int baseSizeY, int baseSizeZ,
      ...
      if (resLevels.getLast().getImageNumBytes() > 4*BYTES_PER_MB) {
         ...
         double xRatio = imageSizeX / Math.sqrt(imageSizeZ*imageSizeY/100);
         double yRatio = imageSizeY / Math.sqrt(imageSizeZ*imageSizeX/100);
         double zRatio = imageSizeZ / Math.sqrt(imageSizeX*imageSizeY/100);         
         boolean reduceX = xRatio >1;
         boolean reduceY = yRatio > 1;
         boolean reduceZ = zRatio > 1;             
         if(!reduceX && !reduceY && !reduceZ) {
             if (zRatio <= xRatio && zRatio <= yRatio) {
                 reduceZ = true;
